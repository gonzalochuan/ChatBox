...
export default function TeacherBannersPage() {
  ...
  const [audience, setAudience] = useState<{
    ...
    subjects: string[];
    students: { id: string; name: string | null; email: string; studentId: string | null }[];
  } | null>(null);
  ...
  const load = async () => {
    ...
    try {
      ...
      const res = await fetch(`${apiBase}/teacher/banners`, {
  ...
  const loadAudience = async () => {
    try {
      const token = getToken();
      if (!token) throw new Error("no_token");
      const res = await fetch(`${apiBase}/teacher/audience`, {
        headers: { Authorization: `Bearer ${token}` },
        cache: "no-store",
      });
      ...
      setAudience({ section: null, subjects: [], students: [] });
    ...
    <div className="app-theme relative min-h-dvh text-white bg-black">
      ...
      <div className="relative z-10 h-dvh grid grid-rows-[64px_1fr] min-h-0">
        ...
        <div className="p-3 md:p-4 space-y-3 overflow-y-auto">
          ...
          {loading ? (
            <div className="rounded-xl border border-white/15 bg-black/40 backdrop-blur-sm px-4 py-6 text-white/70 text-sm">Loading…</div>
          ) : banners.length === 0 ? (
            <div className="rounded-xl border border-white/15 bg-black/40 backdrop-blur-sm px-4 py-6 text-white/60 text-sm">
              No banners yet. Create one to broadcast an alert to your section.
            </div>
          ) : (
              ...
              <div className="grid grid-cols-1 divide-y divide-white/10">
                {banners.map((banner) => (
                  <BannerRow
                    key={banner.id}
                    banner={banner}
                    onEdit={async () => { await loadAudience(); setEditing(banner); }}
                    onDelete={() => setConfirmDelete(banner)}
                    onToggle={async () => {
                      try {
                        const token = getToken();
                        if (!token) throw new Error("no_token");
                        await fetch(`${apiBase}/teacher/banners/${banner.id}`, {
                          method: "PATCH",
                          headers: {
        ...
        <BannerModal
          ...
          onSubmit={async (payload) => {
            try {
              const token = getToken();
              if (!token) throw new Error("no_token");
              await fetch(`${apiBase}/teacher/banners`, {
                method: "POST",
                headers: {
                  "Content-Type": "application/json",
                  Authorization: `Bearer ${token}`,
        ...
        <BannerModal
          ...
          onSubmit={async (payload) => {
            try {
              const token = getToken();
              if (!token) throw new Error("no_token");
              await fetch(`${apiBase}/teacher/banners/${editing.id}`, {
                method: "PATCH",
                headers: {
                  "Content-Type": "application/json",
                  Authorization: `Bearer ${token}`,
        ...
        <ConfirmModal
          ...
          onConfirm={async () => {
            try {
              const token = getToken();
              if (!token) throw new Error("no_token");
              await fetch(`${apiBase}/teacher/banners/${confirmDelete.id}`, {
                method: "DELETE",
                headers: { Authorization: `Bearer ${token}` },
              });
    ...
    <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_auto] gap-4 xl:gap-8 px-4 sm:px-6 py-5">
      <div className="space-y-2">
        <div className="flex flex-wrap items-center gap-2 text-sm">
          ...
          <span className="rounded-full border border-blue-400/30 bg-blue-500/10 px-2.5 py-0.5 text-[11px] text-blue-200">
            Audience: My Section
          </span>
          {banner.isActive && (
...
function BannerModal({
  ...
  onSubmit: (payload: {
    ...
    startsAt: string | null;
    endsAt: string | null;
    audience: { type: "section" } | { type: "subject"; values: string[] } | { type: "users"; values: string[] };
  }) => Promise<void>;
}) {
  ...
  const [error, setError] = useState<string | null>(null);
  const [audType, setAudType] = useState<"section" | "subject" | "users">("section");
  const [audSubjects, setAudSubjects] = useState<string[]>([]);
  const [audUsers, setAudUsers] = useState<string[]>([]);
  ...
  const submit = async () => {
    try {
      ...
      let audience: { type: "section" } | { type: "subject"; values: string[] } | { type: "users"; values: string[] } = { type: "section" };
      if (audType === "subject") {
        if (audSubjects.length === 0) throw new Error("Select at least one subject");
        audience = { type: "subject", values: audSubjects };
      } else if (audType === "users") {
        if (audUsers.length === 0) throw new Error("Select at least one student");
        audience = { type: "users", values: audUsers };
      }
      await onSubmit({
        title: name.trim(),
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
      <div className="w-full max-w-2xl rounded-2xl border border-white/15 bg-white/5 backdrop-blur-md p-4">
        ...
        <div className="max-h-[70vh] overflow-y-auto pr-1 space-y-4">
          ...
          />
          <div className="space-y-2">
            <div className="text-[11px] text-white/60">Audience</div>
            <div className="flex flex-wrap gap-2 text-xs">
              <button
                type="button"
                onClick={() => setAudType("section")}
                className={`rounded-full px-3 py-1 border ${audType === "section" ? "border-white/60 bg-white/10" : "border-white/20 bg-white/5"}`}
              >
                My Section{audienceData?.section ? ` (${audienceData.section.name})` : ""}
              </button>
              <button
                type="button"
                onClick={() => setAudType("subject")}
                className={`rounded-full px-3 py-1 border ${audType === "subject" ? "border-white/60 bg-white/10" : "border-white/20 bg-white/5"}`}
              >
                Subject Groups
              </button>
              <button
                type="button"
                onClick={() => setAudType("users")}
                className={`rounded-full px-3 py-1 border ${audType === "users" ? "border-white/60 bg-white/10" : "border-white/20 bg-white/5"}`}
              >
                Specific Students
              </button>
            </div>
            {audType === "subject" && (
              <div className="rounded-xl border border-white/15 p-2">
                <div className="text-[11px] text-white/60 mb-2">Select subjects</div>
                <div className="flex flex-wrap gap-2">
                  {(audienceData?.subjects || []).map((code) => (
                    <label key={code} className="inline-flex items-center gap-2 text-xs">
                      <input
                        type="checkbox"
                        checked={audSubjects.includes(code)}
                        onChange={(e) =>
                          setAudSubjects((prev) =>
            ...
            {audType === "users" && (
              <div className="rounded-xl border border-white/15 p-2 max-h-56 overflow-auto">
                <div className="text-[11px] text-white/60 mb-2">Select students</div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                  {(audienceData?.students || []).map((s) => (
                    <label key={s.id} className="inline-flex items-center gap-2 text-xs">
                      <input
                        type="checkbox"
                        checked={audUsers.includes(s.id)}
                        onChange={(e) =>
                          setAudUsers((prev) =>
                      ...
                      />
                      <span>{s.name || s.email}{s.studentId ? ` • ${s.studentId}` : ""}</span>
                    </label>
                  ))}
                </div>
              </div>
